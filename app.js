...
function addCard(name, i, cls=''){const fig=document.createElement('figure');fig.className='media-card '+cls;const img=document.createElement('img');img.loading='lazy';img.src='assets/'+encodeURIComponent(name).replace(/%2F/g,'/');img.alt='Gorilla Boomin media '+(i+1);img.onclick=()=>openLightbox(img.src,img.alt);fig.appendChild(img);archive.appendChild(fig);}
ARCHIVE.slice(0,48).forEach((n,i)=>addCard(n,i));
const studioGrid=$('#studioGrid'); STUDIO.forEach((n,i)=>{const img=document.createElement('img');img.loading='lazy';img.src='assets/'+encodeURIComponent(n).replace(/%2F/g,'/');img.alt='BOOM STUDIO '+(i+1);img.onclick=()=>openLightbox(img.src,img.alt);studioGrid.appendChild(img)});
const videoGrid=$('#videoGrid'); VIDEOS.forEach((n,i)=>{const card=document.createElement('article');card.className='video-card';card.innerHTML=`<div class="video-wrap"><video controls preload="metadata" src="assets/${encodeURIComponent(n)}"></video></div><div class="video-copy"><span>BOOM MEDIA • VIDEO ${String(i+1).padStart(2,'0')}</span><h3>${i===0?'THE BOOM FILES':'BEHIND THE BOOM'}</h3><p>Original Gorilla Boomin media archive.</p></div>`;videoGrid.appendChild(card)});
